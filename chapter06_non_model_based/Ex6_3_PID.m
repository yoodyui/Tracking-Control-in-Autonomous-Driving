% Example 6.3: PID Controller Implementation for Lateral Control
clear; clc; close all;
...
Ki = 0;    % Integral gain (set to zero for simplicity)
Kd = 0; % Derivative gain
Kt = 1;  % Heading error gain
...
for k = 1:N
    ...
    if k == 1
        u(2, k) = Kp * e(k) + Ki * integral_error + Kt * theta_e(k);
    else
        de = (e(k) - e(k-1)) / Ts;
        integral_error = integral_error + e(k)*Ts;
        u(2, k) = Kp * e(k) + Ki * integral_error + Kd * (e(k) - e(k-1)) / Ts + Kt * theta_e(k);
    end
...
legend('Reference','PID','Location','best');

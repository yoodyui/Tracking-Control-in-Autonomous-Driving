%Example 4.1 Comparison of dynamic and kinematic models
...
vx0 = 1e-3;      % Initial longitudinal velocity (m/s)
...
%% Simulation Time
T_final = 30;  % Simulation time (s)
Tspan = [0 T_final];

%% Control Inputs (Constant Inputs for Demonstration)
a = 2;                %Acceleration
delta = 5 * pi / 180;  % Steering angle (rad)
Sf = m * a / (2 * (Cxf * cos(delta) + Cxr)); % Front slip ratio (To match a=1, S = m * a / (2 * (Cxf * cos(delta) + Cxr)))
Sr = m * a / (2 * (Cxf * cos(delta) + Cxr)); % Rear slip ratio (Same as Sf to match acceleration)
U = [delta; Sf; Sr];   % Control input vector

%% Solve Dynamic Model using ODE45
...
[t_k, X_k] = ode45(@(t, X) kinematic_bicycle_model_2inputs(t, X, [a; delta], Lf+Lr), Tspan, [x0; y0; psi0; vx0]);

%% Extract Results
x_k = X_k(:,1);   % X position
y_k = X_k(:,2);   % Y position
psi_k = X_k(:,3); % Yaw angle
vx_k = X_k(:,4); % Velocity angle
...
legend('Kinematic','Dynamic');
grid on;

figure;
subplot(2,1,1); hold on;
plot(t, 0*vy, 'b--', 'LineWidth', 2);
...
figure; 
subplot(2,1,1); hold on;
plot(t_k, psi_k * (180/pi), 'b--', 'LineWidth', 2);
...
subplot(2,1,2); hold on;
plot(t_k, [X_k(1,3); diff(X_k(:,3))./diff(t_k)], 'b--', 'LineWidth', 2)
